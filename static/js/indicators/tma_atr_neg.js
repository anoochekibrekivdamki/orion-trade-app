...
/**
 * Рассчитывает TMA_DOWN для каждой свечи так, чтобы последняя свеча была включена в расчёт.
 ...
 */
export function calculateTMA_DOWN(data, periodTMA = 100, periodATR = 110, multiplier = 6) {
    if (!data || data.length === 0) return [];

    const tma = calculateTRIMA(data, periodTMA); // TRIMA для каждой свечи
    const atr = calculateATR(data, periodATR);   // ATR для каждой свечи
    const result = [];
